...
class AudioEffectManager {
	...
	public func startPlayMusic(call: FlutterMethodCall, result: @escaping FlutterResult) {
		if let musicParam = CommonUtils.getParamByKey(call: call, result: result, param: "musicParam") as? String {
			...
			txAudioEffectManager.startPlayMusic(param, onStart: {
				...
				TencentTRTCCloudBeauty.invokeListener(type: ListenerType.onMusicObserverStart, params: ["id": param.id, "errCode": errCode]);
				
			}, onProgress: {
				(progressMs, durationMs) -> Void in
				
				TencentTRTCCloudBeauty.invokeListener(type: ListenerType.onMusicObserverPlayProgress, params: ["id": param.id, "curPtsMS": progressMs, "durationMS": durationMs]);
				
			}, onComplete: {
				(errCode) -> Void in
				
				TencentTRTCCloudBeauty.invokeListener(type: ListenerType.onMusicObserverComplete, params: ["id": param.id, "errCode": errCode]);
			});
			result(nil);
		}
	}
	...
	*/
	public func setMusicPitch(call: FlutterMethodCall, result: @escaping FlutterResult) {
		if let volume = CommonUtils.getParamByKey(call: call, result: result, param: "volume") as? Int32,
