		   ...
		   let path = CommonUtils.getParamByKey(call: call, result: result, param: "path") as? String {
			...
			txCloudManager.snapshotVideo(userId, type: TRTCVideoStreamType(rawValue: streamType)!, completionBlock: {
				...
				do {
					...
					TencentTRTCCloudBeauty.invokeListener(type: ListenerType.onSnapshotComplete, params: ["errCode": 0, "path": path]);
				} catch {
					CommonUtils.logError(call: call, errCode: -1, errMsg: "\(error)")
					TencentTRTCCloudBeauty.invokeListener(type: ListenerType.onSnapshotComplete, params: ["errCode": -1, "errMsg": "\(error)", "path": nil]);
				}
			});
			result(nil);
		}
